...
// folder must contains: header.key  masks2.key  masks.key  name.key  primary2.key  primary.key
// original repo https://github.com/kov-serg/get-cpcert/blob/master/get-cpcert.c
...
typedef struct {
  ...
  unsigned char mask[max_key_size], prim[max_key_size], priv[max_key_size],
    pub8[8], salt[12], *cert;
  const char *password;
...
static int extract_priv(my_data_t *data) {
  ...
  if (data->alg==ALG_GOST2012_512) {
    rc=make2012_pwd_key(pwd_key, data->salt,12, data->password); ec=1;          if (rc) { ERROR("pass"); rc=20; goto err; }
    data->key_size=64;
    data->ecparam_id=NID_id_tc26_gost_3410_2012_512_paramSetA;
  } else
  { ERROR("unsupported alg"); rc=1; goto err; }
  key_with_mask=decode_primary_key(pwd_key,data->prim,ctx,ec,data->key_size);   if (!key_with_mask) { ERROR("decode"); rc=21; goto err; }
  OPENSSL_cleanse(pwd_key,sizeof(pwd_key));
  BUF_reverse(buf,data->mask,data->key_size);
  mask=BN_bin2bn(buf,data->key_size,mask);                                      if (!mask) { ERROR("bin2nb"); rc=22; goto err; }
  raw_key=remove_mask_and_check_public(key_with_mask,mask,data->pub8,
    data->key_size,ctx,data->ecparam_id);                                       if (!raw_key) { ERROR("invalid password"); rc=23; goto err; }
...
int get_cpcert(const char* path,const char* pass) {
  ...
  pkt_init(primary,buf1,buf1_size);
